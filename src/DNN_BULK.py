...
data_dir = '/Users/derekwang/Work/TLR_ML/Data/ONEMONTHCLEAN.csv'
...
#Create randomly selected train data set and cross validation data set

     
df_train, df_cross = rand_datasets(data_clean, 0.8)

# df_train.to_csv('Train_set.csv')
# df_cross.to_csv('CrossVal_set.csv')

train_features = df_train.iloc[:,2:25]
X = train_features.values
train_labels = df_train['OVERAGE_FLG']
y = train_labels.values



test_features = df_cross.iloc[:,2:25]
X_test = test_features.values
test_labels = df_cross['OVERAGE_FLG']
...
final_result.to_csv('SVC1.csv', index = False)

#y_pred = clf_dnn.predict(X_test_scaled)
...
#print("F1 score on corss_val is %s" % f1_score)
#recall_score = sklearn.metrics.recall_score(y_test, y_pred)
#print("Recall score on corss_val is %s" % recall_score)
#precision_score = sklearn.metrics.precision_score(y_test, y_pred)
#print("Precision score on corss_val is %s" % precision_score)
